...
public interface PresencaRepository extends JpaRepository<Presenca, Long> {
    ...
    @Query("SELECT p FROM Presenca p WHERE p.usuario.id = :usuarioId AND p.data = :data")
    Presenca findByUsuarioIdAndData(@Param("usuarioId") Long usuarioId, @Param("data") LocalDate data);

    @Query("SELECT p FROM Presenca p WHERE p.usuario.id = :usuarioId AND p.viagem.id = :viagemId AND p.data = :data")
